...
private:
    ...
    struct Node
    {
         ...
         * 右值引用是一种引用类型，允许你获取和修改右值（通常是临时对象）的资源。它通过使用 && 语法来声明：int&& rvalueRef = 10;
         * 使用 std::move(d) 将 d 转换为右值，从而调用 Object 的移动构造函数。这可以避免不必要的复制，提高性能
         * @param d 节点内存放的数据. 只有这里是一个显式的右值引用，才会调用这个构造函数.
         * @param p 前一个节点的指针. 默认为空.
         * @param n 后一个节点的指针. 默认为空.
         */
        Node(Object &&d, Node *p = nullptr, Node *n = nullptr)
            : data{std::move(d)}, prev{p}, next{n} {}     //成员初始化列表，带动该构造函数的调用
    };
...
public:
    ...
    public:
        ...
        const_iterator &operator++()
        {
            ...
            return *this;                //this是指向当前迭代器的指针，返回当前迭代器，返回对象的引用，支持连续自增操作++（++i），不用创建临时对象
        }
        ...
        const_iterator operator++(int)
        {
            ...
            ++(*this);
            return old;                 //返回一个临时对象old，表示自增前的状态，返回的是对象old的副本，理论上这里返回old的副本是调用了拷贝构造函数的，但现代编译器中会优化来避免不必要的拷贝构造
        }

        /// 这里我们看到前自增和后自增的区别. 就是 std::cout << *++iter << std::endl;
        /// 和 std::cout << *iter++ << std::endl; 的区别. 前者是先自增，再返回增加后的值；后者是先返回当前值，再自增.
    ...
    protected:
        ...
        /// 注意到 const_iterator 并没有提供析构函数，因为它不需要也不应该释放内存.  没有动态内存管理，new delete，有默认析构函数就足够了
    };
    ...
    public:
         ...
         * 但这样会使代码更加清晰.
         * 
         * 前面一个const指返回的是一个常量引用const object&，意味着不能通过这个引用修改所指向的对象
         * 后面一个const是指这是一个常量成员函数，该函数不会修改所属对象的成员变量，可在const对象上调用
         * @return const Object& 
         */
        const Object &operator*() const
        {
         ...
         * @return iterator 下一个节点的迭代器.
         */
        iterator operator++(int)  //（int）是专门为了区分前置自增和后置自增，就是这样用的
        {
            iterator old = *this;
            ++(*this);
            return old;    //返回旧的迭代器，并不是引用的方式，因为只是值返回旧的，本身已经改变了
        }
...
public:
    ...
    List(const List &rhs)
    {
        ...
        /// 而这里的 for 循环的方式实际上让我们可以遍历一个容器，而不需要知道容器的具体类型.  //list是一个容器，内部有begin和end的迭代器
     ...
     */
    List &operator=(List copy)                                            //值传递，这里的copy是传入的这个数据的副本，调用了拷贝构造函数 ，注意这里的拷贝构造是深度复制了一个副本出来
    {
    //    std::swap( *this, copy );
    ...
    iterator begin()
    {
        ...
        //在双向链表中，哨兵节点 head 的 prev 和 next 指针分别指向链表的最后一个和第一个实际数据节点
        return { head->next };
    }
    ...
    {
        return { head->next };                           //head是list的一个指针，指向第一个元素之前，所以其->next指向第一个元素
    }
    ...
    iterator end()
    {
        return { tail };                                //tail是list的一个指针，指向最后一个元素之后，所以其->prev 指向最后一个元素
    }
    ...
    void clear()
    {
        /// 这里其实用计数器来判断 List 是否为空.  //一直不断删除第一个节点
        while (!empty())
            pop_front();
    }
    ...
    Object &front()
    {
        return *begin();   //return begin()->data;???     begin指向第一个节点
    }
    ...
    {
        Node *p = itr.current;              //获取当前迭代器指向的节点
        theSize++;                          //增加链表的节点计数
        /// 仔细想一下这个过程.
        return { p->prev = p->prev->next = new Node{ x, p->prev, p } };
        //首先，每一个等号返回的都是右端项；；
        //从右端开始，new Node{ x, p->prev, p }建立一个新节点，前驱为p->prev，后继为p，
        //然后将这个新节点插入到上一个节点和当前节点之间：p->prev->next = new ... :安排上一个节点的next指向新节点 。（返回的仍然是这个新节点）
        //最后安排当前节点的prev指向新节点 （最后仍然返回新节点）
        //总体来说，就是找到插入位置后，先建立新节点，再安排上一个节点，再安排新节点与当前节点的关系
    }
    ...
    {
        Node *p = itr.current;
        iterator retVal{ p->next };  //初始化，，记录下来当前位置的下一个指针，因为后面要把p删去就找不到了
        p->prev->next = p->next;     //安排上一个位置的next指针
        p->next->prev = p->prev;     //安排下一个位置的prev指针
        delete p;
        theSize--;
        return retVal;
    }
